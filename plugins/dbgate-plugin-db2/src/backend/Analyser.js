...
class Analyser extends DatabaseAnalyser {
  ...
  _ensureUniqueColumnNames(columns, tableInfo = {}) {
    ...
    console.log(`[DB2 Analyser] Checking uniqueness for ${columns.length} columns for table ${tableInfo.pureName || tableInfo.tableName}`);
    const uniqueNameMap = {};
    const result = [];
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      column.ordinalPosition = column.ordinalPosition || column.COLNO || column.colno || i;
    ...
    if (duplicates.length > 0) {
      console.warn(`[DB2 Analyser] Fixed ${duplicates.length} duplicate column names`);
    }
  ...
  async _runAnalysis() {
    ...
    try {
      ...
      for (const schema of schemasArray) {
        ...
        try {
          ...
          result.procedures = result.procedures.concat(structureData.procedures);
          
          // Add triggers from this schema to result
          result.triggers = result.triggers.concat(structureData.triggers || []);        } catch (schemaErr) {
          console.error(`[DB2] Error analyzing schema ${schema.name}: ${schemaErr.message}`);
          // Fallback to direct table, view, function and procedure retrieval
          try {
            const schemaName = schema.name;
            console.log(`[DB2] Falling back to direct retrieval methods for schema ${schemaName}`);
            
            // Get tables
            const tables = await this.getTables(schemaName);
            const validTables = tables.filter(table => table && table.schemaName && table.pureName);
            console.log(`[DB2] Found ${tables.length} tables in schema ${schemaName}, ${validTables.length} valid`);
            
            // For each table, try to retrieve columns
            for (const table of validTables) {
              try {
                console.log(`[DB2] Fetching columns for table ${table.schemaName}.${table.pureName}`);
                const columns = await this.getColumns(table);
                
                // If we got columns, add them to the table
                if (columns && columns.length > 0) {
                  table.columns = columns;
                  console.log(`[DB2] Added ${columns.length} columns to table ${table.pureName}`);
                }
              } catch (columnErr) {
                console.error(`[DB2] Error getting columns for table ${table.pureName}: ${columnErr.message}`);
              }
            }
            ...
            console.log(`[DB2] Found ${triggers.length} triggers in schema ${schemaName}`);
    ...
    try {
      ...
      try {
        ...
        console.error(`[DB2] Error querying SYSCAT.SCHEMATA with details: ${err.message}`);
      ...
      try {
        ...
        if (res.rows && res.rows.length > 0) {
          ...
          console.log(`[DB2] Found ${schemas.length} schemas from basic SYSCAT.SCHEMATA query`);
          return schemas;
        }
      } catch (err) {
        console.error(`[DB2] Error querying basic SYSCAT.SCHEMATA: ${err.message}`);
      }
  ...
  async incrementalAnalysis(structure) {
    ...
    try {
      ...
      try {
        ...
        console.log(`[DB2] Found ${res.rows?.length || 0} columns using SYSCAT.COLUMNS`);
        ...
        return (res.rows || []).map((row, index) => {
          ...
          const colName = row.COLNAME || row.colname || row.columnName || `column_${index}`;
          ...
          normalizedRow.uniqueName = `${table.schemaName}.${table.tableName || table.pureName}.${colName}_${index}`;
        ...
        try {
          ...
          console.log(`[DB2] Found ${res.rows?.length || 0} columns using SYSIBM.SYSCOLUMNS`);
          
          // Process the rows to normalize case sensitivity and ensure unique identifiers
          return (res.rows || []).map((row, index) => {
            const normalizedRow = {};
            // Get column name with fallbacks
            const colName = row.NAME || row.name || row.columnName || `column_${index}`;
            ...
            try {
              ...
              console.log(`[DB2] Found ${res.rows?.length || 0} columns using INFORMATION_SCHEMA.COLUMNS`);
              
              // Process the results to ensure unique identifiers
              return (res.rows || []).map((row, index) => {
                // Ensure we have a uniqueName for each column
                const colName = row.COLUMN_NAME || row.columnName || `column_${index}`;
                return {
                  ...row,
                  columnName: colName,
                  uniqueName: `${table.schemaName}.${table.tableName || table.pureName}.${colName}_${index}`
                };
              });
          } catch (lastResortErr) {
  ...
  async getTriggers(schemaName) {
    try {
      console.log(`[DB2] Getting triggers for schema: ${schemaName || 'current schema'}`);
      ...
      try {
        ...
        const triggersRes = await this.driver.query(this.connection, triggersQuery, [effectiveSchema]);
        ...
        console.log(`[DB2] Successfully retrieved ${triggers.length} triggers from SYSCAT.TRIGGERS`);
      } catch (err) {
        console.error(`[DB2] Error getting triggers from SYSCAT.TRIGGERS: ${err.message}`);
        ...
        try {
          ...
          const triggersRes2 = await this.driver.query(this.connection, triggersQuery2, [effectiveSchema]);
          ...
          if (triggersRes2.rows && triggersRes2.rows.length > 0) {
            triggers = triggersRes2.rows.map(row => {
              ...
              return {
                ...
                objectType: 'trigger',
                objectId: `${(row.SCHEMA || row.schemaName || '').trim()}.${triggerName}`,
                description: row.REMARKS || row.description,
                definition: row.TEXT || row.definition,
          ...
          console.log(`[DB2] Successfully retrieved ${triggers.length} triggers from SYSIBM.SYSTRIGGERS`);
        } catch (err2) {
          console.error(`[DB2] Error getting triggers from SYSIBM.SYSTRIGGERS: ${err2.message}`);
